...
import choicePageReducer from "./choicePageReducer.js";
import sortTableReducer from "./sortTableReducer.js";
...
let store = {
    ...
    //Данные о пользователях, всех и только отображенных в таблице на данной странице
    _state : {
        users: [
            {
    ...
    //Все редьюсеры изменяют state посредством созданию нового объекта state и заменой им старого
    dispatch(action){
        this._state = editReducer(action,this._state);
        this._state = choicePageReducer(action,this._state);
        this._state = sortTableReducer(action,this._state);
    },

    //Сюда передается функция по перерисовке строк таблицы
    subscribe(observer) {
