...
int main() {
    field A();///шаг алгоритма, время движения частицы
    A.setElectricStrength();///Напряженность поля в проекциях координат X,Y,Z соответственно
    A.setCoord();///Начальные координаты частицы на X,Y,Z соответственно
    A.setInduction();///Индукция поля в проекциях координат X,Y,Z соответственно
    A.setSpeed();///Начальная скорость частицы на X,Y,Z соответственно
    A.odnorodnoe();
    return 0;
}
